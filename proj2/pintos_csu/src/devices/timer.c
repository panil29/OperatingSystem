...
static struct list thr_sleep_list;
...
/*this fucntion will compare the ticks to place it in ascending ordered under timer_sleep*/
bool tick_comparator(struct list_elem *a, struct list_elem *b, void *aux)
{
  struct thread *x = list_entry(a, struct thread, elem); 
  struct thread *y = list_entry(b, struct thread, elem);
  if(x->wakeup_t < y->wakeup_t)//wakeup_t is a field in struct thread which i have added
   return true;
  else
   return false;
}
...
timer_sleep (int64_t ticks) 
{
  ...
  thread_current()->wakeup_t = ticks + start;
